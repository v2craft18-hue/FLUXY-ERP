  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [pedidos, clientes, meta] = await Promise.all([
        supabase.from('pedidos').select('id, total, status, data, numero, cliente:clientes(nome)').order('data', { ascending: false }).limit(100),
      ...
      const fatHoje = peds.filter(p => p.data?.slice(0,10) === hoje && ok(p)).reduce((a, p) => a + (p.total || 0), 0)
      const fatMes  = peds.filter(p => p.data?.slice(0,7) === mes && ok(p)).reduce((a, p) => a + (p.total || 0), 0)
      ...
      const grafico = Array.from({ length: 7 }, (_, i) => {
        ...
        const fat = peds.filter(p => p.data?.slice(0,10) === d && ok(p)).reduce((a, p) => a + (p.total || 0), 0)
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {data.pedidosRecentes.map((p: any) => {
